...
function ChangeValue(first, index, value) {
    if ((_gdata[index].count + value) >= _gdata[index].min && (_gdata[index].count + value) <= _gdata[index].max) {
        ...
        if (!first) {
            ...
            $.post('https://utility_framework/slider', data);
        }
...
window.addEventListener('message', function(event){	
    ...
    if (data.item == true) {
        $(".notify").append(`
            <li class="item">
                <img src="nui://${imgDir}/${data.name}.png" width="60" height="60">
                <span>${data.changed}</span>
            </li>
        `);

        setTimeout(() => {
            $(".notify > :first-child").remove();
        }, 5000);

    } else if (data.imgdir != undefined) {
        imgDir = data.imgdir
    } else {
        if (data.clipboard) {
            ...
            $("#import").keypress(function( event ) {
                if ( event.which == 13 ) {
                    ...
                    $.post('https://utility_framework/DialogData', JSON.stringify(
                        {
                            "text":$('#import').val()
                        }
                    ));
...
document.onkeyup = function (data) {
	if (data.which == 27) {
		$.post('https://utility_framework/close', '{}');
	}
};
...
function GenerateMenu(data, closeLabel, title) {
    ...
    $(".menu_option").click(function(event){
        if (event.target.id == "close") {
            $.post('https://utility_framework/backsubmenu', '{}');
        } else {
            $.post('https://utility_framework/button_selection', $('#'+event.target.id).attr('_data'));
        }
    })
};
