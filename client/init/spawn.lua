...
AddEventHandler('onClientMapStart', function()
    local uPlayer = LocalPlayer.state

    exports.spawnmanager:setAutoSpawn(false)

    while uPlayer.other_info == nil do
    ...
    exports.spawnmanager:spawnPlayer({
        ...
        model = `mp_m_freemode_01`, -- dont know if works
        skipFade = false
    }, function() print("[DEBUG] [SKIN] Spawned player from spawnmanager") end)
        ...
        for i=1, #Config.Identity do
            if Config.Identity[i] == "sex" then
                LoadMpPlayer(uPlayer.identity[i] == "f", function()
                    ...
                    if uPlayer.other_info.isdeath then
